"""Task pipeline for M-038.

This module is a thin wrapper that delegates all real work to the vendored
phase2 logic in ``src/pipeline/_phase2/m038_isic.py``. The phase2 module
generates the standard 7-file task layout into
``datasets/_example_output/M-038_isic2018_skin_lesion_seg/`` relative to the pipeline's
``DATA_ROOT``.
...
import importlib
_phase2_mod = importlib.import_module("m113_bus")
